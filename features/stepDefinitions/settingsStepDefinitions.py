...
@step(u"I am in the global settings tab")
def verifyGlobalSettings(step):
    assert settings.verifyGlobalSettingsTab()


@step(u"I am in the variant filter tab")
def verifyVariantFilter(step):
    assert settings.verifyVariantFilterTab()


@step(u"I am under Variant filter in the \[(.*)\] tab")
def verifyInSpecificTab(step, tabName):
    assert settings.verifyInSpecificTabOfVariantFilter(tabName)
...
@step("I expand the details with org unit \[(.*)\] and labtest configuration \[(.*)\]")
